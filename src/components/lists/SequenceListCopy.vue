          ...
          <template v-if="!isLoading && isListVisible">
            ...
            >
              ...
              >
                <div class="flexrow">
                  <input
                    type="checkbox"
                    class="mr1"
                    :checked="selectedSequences.has(sequence.id) || null"
                    @input="event => toggleLine(sequence, event)"
              ...
              <row-actions-cell
                ...
                "
                @edit-clicked="
                  $emit('edit-clicked', [...selectedSequences.values()], () => {
                    selectedSequences.clear()
                  })
                "
                v-if="isCurrentUserManager"
...
import { range } from '@/lib/time.js'

export default {
  name: 'sequence-list-copy',
  ...
  computed: {
    ...
    isListVisible() {
      return (
        !this.isLoading && !this.isError && this.displayedSequences.length > 0
      )
    },
  ...
  methods: {
    ...
    toggleLine(sequence, event) {
      ...
      const shotsToSelect = [sequence]
      if (this.shiftKeyPressed && this.lastSelectedSequence) {
        const shotsFlatten = this.displayedSequences.flat()
        let startShotIndex = shotsFlatten.findIndex(
          displayedShot => displayedShot.id === this.lastSelectedSequence.id
        )
        let endShotIndex = shotsFlatten.findIndex(
          displayedShot => displayedShot.id === sequence.id
        )
      ...
      this.lastSelectedSequence = sequence
      if (selected) {
        shotsToSelect.forEach(shot => {
          this.selectedSequences.set(shot.id, shot)
        })
      } else {
        shotsToSelect.forEach(shot => {
          this.selectedSequences.delete(shot.id)
        })
      }
    },
