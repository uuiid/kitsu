          ...
          >
            ...
            >
              ...
              <row-actions-cell
                ...
                :entry="sequence"
                @delete-clicked="
                  $emit('delete-clicked', [...selectedShots.values()], () => {
                    selectedShots.clear()
                  })
                "
                @edit-clicked="
                  $emit('edit-clicked', [...selectedShots.values()], () => {
                    selectedShots.clear()
                  })
                "
                v-if="isCurrentUserManager"
...
export default {
  ...
  computed: {
    ...
    isListVisible() {
      return !this.isLoading && !this.isError && this.displayedShots.length > 0
    },
  ...
  methods: {
    ...
    toggleLine(shot, event) {
      ...
      if (this.shiftKeyPressed && this.lastSelectedShot) {
      ...
      if (selected)
        shotsToSelect.forEach(shot => {
          this.selectedShots.set(shot.id, shot)
        })
      else {
        shotsToSelect.forEach(shot => {
          this.selectedShots.delete(shot.id)
        })
      }
    },
    ...
    onInputKeyUp(event, i, j) {
      ...
      const listHeight = this.displayedShots.length
